export const configDir = process.env.XDG_CONFIG_HOME ? `${process.env.XDG_CONFIG_HOME}/rainwall` : `${os.homedir()}/.config/rainwall`
export const cacheDir = process.env.XDG_CACHE_HOME ? `${process.env.XDG_CACHE_HOME}/rainwall` : `${os.homedir()}/.cache/rainwall`
...
export async function loadConfig(pathToConfig: string, defaultConfig: object) {
    ...
    try {
        const stringConfig = await fs.promises.readFile(`${pathToConfig}`, { encoding: 'utf8' })
        if (stringConfig == undefined) {
            console.warn(`Current config blank, generating a new one... you may need to stop this process and edit the config file.`)
